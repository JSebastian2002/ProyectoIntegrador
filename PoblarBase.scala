...
object PoblarBase extends App {
  ...
  //Tabla Relacion: Production_Companies - Pelicula
  ...
  //Relacion: Pelicula-Crew
  /*
    val crewfix = crew
    val pelicrew = data
      .map(row => (row("id"), crewfix))
      .map(row => (row._1, row._2.map(_.get).flatMap(_.as[JsArray].value)
        .map(_.as[JsObject]).map(jsObj => (jsObj("id").as[Int]))))
      .flatMap(x => x._2.map((x._1,_)))
      .map( m => (m._1.toInt,m._2))

        pelicrew.foreach(x =>
        sql"""
                 INSERT INTO pei_crew(id_p2, c_id1)
                 VALUES
                 (${x._1}, ${x._2})
                 """.stripMargin
          .update
